...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    row_time,sum_result,result = 1,0,0
    while row_time<=num_rolls:
        roll_result = dice()
        if roll_result == 1:
            result = 1
        sum_result = sum_result + roll_result
        row_time+=1
    if result == 1:
        return 1
    else:
...
def always_roll(n):
    ...
    assert n >= 0 and n <= 10
    # BEGIN PROBLEM 6
    def roll_time(score, oppenent_score):
        return n
    return roll_time
    # END PROBLEM 6
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged_maker(*args):
        ...
        while i<=total_samples:
            sum_roll +=original_function(*args)
            i+=1
        return sum_roll / total_samples
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    avrager = make_averaged(roll_dice,total_samples)
    list = [avrager(i,dice) for i in range(1,11)]
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    new_score = square_update(0, score, opponent_score, dice=six_sided)
    if new_score >= 100:
        return 0
    elif 100 - new_score <=3:
        return 1
    averager = make_averaged(roll_dice, total_samples=100)
    if  new_score- score >= averager(6):
        return 0
